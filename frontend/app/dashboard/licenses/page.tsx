...
export default function LicensesModule() {
  ...
  const handleCreateFlow = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!clientRes.ok || !clientData.success) {
        alert(clientData.message || 'Hubo un problema registrando el cliente. Podría existir ya el correo.');
        setIsCreating(false);
        return;
      }
      ...
      if (!licRes.ok || !licData.success) {
        throw new Error(licData.message || 'Error en validación de licencia');
      }
  ...
  const renderCreateModal = () => {
    ...
    const modalContent = (
      <div onClick={() => setIsCreateModalOpen(false)} className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center animate-in fade-in duration-200 lg:p-4">
        <div onClick={(e) => e.stopPropagation()} className="bg-[#18181B] border border-[#27272a] rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar">
          ...
          <form onSubmit={handleCreateFlow} className="p-6 space-y-6">
            ...
            <div className="pt-2 flex justify-end gap-3 sticky bottom-0 bg-[#18181B] border-t border-[#27272a] p-4 -mx-6 -mb-6 rounded-b-2xl shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
              <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-5 py-2.5 rounded-lg text-sm font-semibold border border-[#27272a] text-[#F8FAFC]/60 hover:text-white hover:bg-[#27272a]/50 transition-colors">Cancelar</button>
  ...
  const renderEditModal = () => {
    ...
    const modalContent = (
      <div onClick={() => setIsEditModalOpen(false)} className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center animate-in fade-in duration-200">
          ...
          <form onSubmit={handleUpdateClient} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={editingClient.phone_number || ''} onChange={e => setEditingClient({ ...editingClient, phone_number: e.target.value })} className="w-full bg-[#09090B] border border-[#27272a] rounded-lg p-2.5 text-sm outline-none focus:border-[#1D427F] text-white" />
              </div>
            </div>
            <div>
